...
namespace InternetData
{
    ...
    public class Example
    {
        ...
        public static Example LoadExample() //example (datatype)
        {
            // Prepare to read the Example.json
            // FileStream is the object that we use to acomplish this task
            //file stream allows to read the method
            //2nd parameter-mode which we are using :"open" open means only reading file, cannot write in it
            FileStream file = new FileStream("/Users/folder.amelia/Desktop/ExampleJSON", FileMode.Open);//file
            ...
            //"black box"-takes JSON text and turn it into object or vice versa
            //needs to tell type of data serialzing "Example"-structure of data that's being read 
            DataContractJsonSerializer serializer = new DataContractJsonSerializer(typeof(Example));//serializer

            // Now, we use the serializer object to Read the text file and convert it
            // into an Instance of the Example class.
            //get example from file
            //read object from file
            Example example = (Example)serializer.ReadObject(file);//read object from file

            // Give that example back to the user who asked for it.
            return example;//spit it back to whatever I call the method
        }
